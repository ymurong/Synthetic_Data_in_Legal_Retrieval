...
class TextPreprocessor():
    ...
    def preprocess(self, series, lowercase=True, remove_punct=True,
        ...
        return (series.progress_apply(
            lambda text: self.preprocess_text(text, lowercase, remove_punct, remove_num, remove_stop, lemmatize)))
...
class BSARDataset(Dataset):
    ...
    def get_id_query_pairs(self, queries: pd.DataFrame) -> Dict[str, str]:
        return queries.set_index('id')['synthetic_question'].to_dict()
    ...
    def get_one_to_many_relevant_pairs(self, queries: pd.DataFrame) -> Dict[str, Set[str]]:
        return queries.set_index('id')['article_ids'].astype(str).str.split(',').apply(
            lambda x: [int(float(i)) for i in x]).to_dict()
    ...
    def get_one_to_one_relevant_pairs(self, queries: pd.DataFrame) -> List[Tuple[int, int]]:
        queries = (queries
                   ...
                   .stack()
                   .reset_index()
                   .drop(['synthetic_question'], axis=1)
