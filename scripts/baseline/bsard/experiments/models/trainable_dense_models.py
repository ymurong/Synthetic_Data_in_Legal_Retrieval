...
class BiEncoder(nn.Module):
    def __init__(self,
                 is_siamese: bool,
                 # whether to train a Siamese (query and document are projected into the same embedding space) or a Dual-tower (query and document are projected into different embedding spaces) BiEncoder model.
                 q_model_name_or_path: str,  # model name (Hugging Face) or path for the query encoder.
                 d_model_name_or_path: Optional[str] = None,
                 # model name (Hugging Face) or path for the document encoder.
                 truncation: bool = False,  # whether to truncate the inputs to a length of 'chunk_size'.
                 max_input_len: Optional[int] = None,
                 # size of the maximum sequence size when truncation is set to True.
                 chunk_size: Optional[int] = None,
                 # size of the truncated input if 'truncation' is True OR size of the split chunks if 'truncation' set to False.
                 window_size: int = 10,
                 # size of the overlapping window between the chunks (only used when 'truncation' is False).
                 pooling_mode: str = 'cls',
                 # strategy to extract a fixed-sized passage embedding out of a variable number of word embeddings (default is set to cls; can also be mean or max).
                 score_fn: str = 'dot',
                 # scoring function between passage embeddings (efault is set to dot product; can also be set to cosine similarity).
    ...
    def collate_batch(self, batch: List[Tuple[str, str]]) -> Tuple[T, T, T, T]:
        ...
        """
        queries, documents = map(list,
                                 zip(*batch))  # Unzip a list of tuples into individual lists: [query, query, ...], [document, document, ...]
        ...
        return {'q_input_ids': q_input_ids, 'q_attention_masks': q_attention_masks, 'd_input_ids': d_input_ids,
...
class PassageEncoder(nn.Module):
    def __init__(self, model_name_or_path: str, truncation: bool, max_input_len: Optional[int],
                 chunk_size: Optional[int], window_size: int, pooling_mode: str):
...
class WordEncoder(nn.Module):
    def __init__(self, model_name_or_path: str, truncation: bool, max_input_len: Optional[int],
                 chunk_size: Optional[int], window_size: int):
    ...
    def tokenize(self, texts: List[str]) -> Tuple[T, T]:
        ...
        """
        tokenized = self.tokenizer(texts, padding=True, truncation=self.truncation, max_length=self.max_input_len,
                                   return_tensors="pt")  # Truncation to 'max_length' AND padding to max sequence in batch (see https://huggingface.co/transformers/preprocessing.html)
    ...
    def _transform_inputs(self, input_ids: T, attention_masks: T) -> Tuple[T, T]:
        ...
        if batch_max_seq_len <= self.chunk_size:
            ...
            input_ids = torch.nn.functional.pad(input=input_ids, pad=(0, batch_seq_len - batch_max_seq_len),
                                                mode='constant', value=self.tokenizer.pad_token_id)
            attention_masks = torch.nn.functional.pad(input=attention_masks, pad=(0, batch_seq_len - batch_max_seq_len),
            ...
            input_ids = input_ids.unfold(dimension=1, size=chunk_size, step=chunk_size - self.window_size).permute(0, 2,
                                                                                                                   1)
            attention_masks = attention_masks.unfold(dimension=1, size=chunk_size,
                                                     step=chunk_size - self.window_size).permute(0, 2, 1)
...
class WordPooler(nn.Module):
    ...
    def forward(self, *inputs: Tuple[T, T]) -> T:
        ...
        if self.pooling_mode == 'cls':
            ...
            return self.pool(token_embeddings=token_embeddings[:, 0, :, :],
            ...
            return self.pool(token_embeddings=token_embeddings[:, 1:, :, :].permute(0, 2, 1, 3).reshape(batch_size, -1,
                                                                                                        embedding_dim),
                             ...
                             attention_masks=attention_masks[:, 1:, :].permute(0, 2, 1).reshape(batch_size, -1),
    ...
    def pool(self, token_embeddings: T, attention_masks: T, pooling_mode: str = 'mean') -> T:
        ...
        if pooling_mode == 'max':
            ...
            sum_mask = attention_masks_expanded.sum(
                dim=1)  # -> Tensor[batch_size, embedding_dim] where each value is the length of the corresponding passage.
            sum_mask = torch.clamp(sum_mask,
                                   min=1e-9)  # Make sure not to have zeros by lower bounding all elements to 1e-9.
